...
#define CACHE_LINE_SIZE 320 //different for each device

// Thread data structure with padding to fill a cache line
typedef struct {
    const int *array; // Array to be processed
    int start;        // Start index for this thread
    int end;          // End index for this thread
    int local_count;  // Local count for this thread
    char padding[CACHE_LINE_SIZE - sizeof(int) - sizeof(int) - sizeof(int) - sizeof(const int*)]; // Padding to fill the cache line
} ThreadData;
